...
а затем значения всех полей всех элементов при помощи метода `method_branch + '.get'`. `method_branch` - группа методов, в которой есть подметоды `*.list` и `*.get`, например `crm.lead` или `crm.deal`*.

*Известно, что группа методов `tasks.task` не может быть использована с методом `list_and_get()`, так как вызов `tasks.task.list` возвращает не плоский, а вложенный список, что не предусмотрено текущей реализацией `list_and_get()`.
