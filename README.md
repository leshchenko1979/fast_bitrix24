...
Вы также можете использовать параметр ```details```, чтобы кастомизировать запрос:
...
```
Метод `post()` возвращает список ответов сервера по каждому элементу переданного списка. 
